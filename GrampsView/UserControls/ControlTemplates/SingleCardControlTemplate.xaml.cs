...
namespace GrampsView.UserControls
{
    public partial class SingleCardControlTemplate : ContentView
    {
        ...
        private void OnTapGestureRecognizerTapped(object sender, TappedEventArgs args)
        {
            try
            {
                Ioc.Default.GetRequiredService<ILog>().Variable("SingleCardControlTemplate-OnTapGestureRecognizerTapped", args.Parameter.ToString(), Microsoft.Extensions.Logging.LogLevel.Trace);

                Navigation.PushAsync((args.Parameter as HLinkBase).NavigationPage());
            }
            catch (Exception ex)
            {
                ErrorInfo t = new ErrorInfo("SingleCardControlTemplate", "OnTapGestureRecognizerTapped")
                {
